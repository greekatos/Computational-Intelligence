...
for i, (train, test) in enumerate(kfold.split(X_tr)):
    model = Sequential()
    # Input Layer
    model.add(Dense(784,kernel_regularizer=l2(0.5), bias_regularizer=l2(0.5), activation='relu', input_dim=784))

    # Second Hidden Layer
    model.add(Dense(1000,kernel_regularizer=l2(0.5), bias_regularizer=l2(0.5), activation='relu'))
